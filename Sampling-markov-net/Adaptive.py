...
#     print(Q)
    ...
    for i in tqdm(range(max(N))):    
        if not i==0 and i % 100 == 0:
            Q = update_Q(Q,wq,samples)
        G_i = copy.deepcopy(G)
        evidance,Q_val,sample = generateSample(G,X,Q)
#         print(evidance)
...
# def update_Q(wq,samples):
def update_Q(Q,wq,samples):
    new_Q = [[0]*len(Q_n) for Q_n in Q]
    deno = sum(wq)
    samples = np.asanyarray(samples)
#     print(sum((samples.T[0] == 0)*np.random.rand(100)))
    for i in range(len(samples[0])):
#         Q.append(np.zeros())
        for j in range(len(Q[i])):
            col = (samples.T[0] == j)
            new_Q[i][j] = (0.6*Q[i][j])+(0.4*sum(col*wq)/deno)
    return new_Q
# update_Q(wq = np.random.rand(100) ,samples = samples)    
...
def generateSample(G,X,Q):
    ...
    return evidance,Q_val,np.asanyarray(sample)
...
for w_cut_size in [1,2,3,4,5]:
    for N in [100,1000,10000,20000]:
        for itteration in range(10):
            ...
            start = time.time()
            G = AdaptiveAlgorithm1(mar,w=w_cut_size,N=N)
            end = time.time()
            total_time = end-start
            with open("adaptive.csv",'a+') as u:
...
def prog(w_cut_size, N, Grid, writefile,itt = 10):
    for itteration in range(itt):
        ...
        Gs,total_time = AdaptiveAlgorithm1(mar,w=w_cut_size,N=N)
...
def prog_generic(w_cut_size, N, inp_file, evid_file, writefile,itt = 10):
    for itteration in range(itt):
        ...
        with open(writefile,'a+') as u:                
            for n in range(len(N)):
                state = inp_file+","+str(w_cut_size)+","+str(N[n])+","+str(itteration)+","+str(Gs[n])+","+str(total_time[n])+",  Adaptive Distribution\n" 
...
for Grid in ['Grids_18','Grids_17','Grids_16','Grids_14','Grids_15']:
    for w_cut_size in [5,4,3,2,1]:
        ...
        prog(w_cut_size,N,Grid,'adaptive.csv',5)
'''
